...
export default {
	...
	computed: {
		...
		availableEntities() {
			return pickerTypeGrouping.map(type => {
				const dataSet = this.searchResults.filter(entity => entity.type === type.id)
				const dataList = [
					{
						id: type.id,
						label: type.label,
	...
	methods: {
		...
		formatResults(result) {
			...
			// TODO: why find doesn't work here?
			// const type = pickerTypeGrouping.find(t => t.type === result.value.shareType).label
			return {
				label: result.label,
