		...
		onCreate() {
			const updateCollective = () => {
				...
				if (this.updatedCollective && this.selectedMembers) {
					// TODO: we most likely need circle member type, not shareType
					const selectedMembers = this.selectedMembers.map(entry => ({
						id: entry.shareWith,
						type: entry.shareType,
					}))
					try {
						this.dispatchAddMembersToCircle({ collective: this.updatedCollective, selectedMembers })
					} catch (e) {
