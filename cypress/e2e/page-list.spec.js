...
describe('Page list', function() {
	...
	it('sorts pages by custom order by default', function() {
		cy.get('.page-list-item').eq(0)
			.should('contain', '#% special chars')
		cy.get('.page-list-item').last()
	...
	it('Moves page to a subpage', function() {
		...
		cy.get('.page-browser .page-list li')
			.contains('Target')
			.click()
		cy.get('.action-buttons button .arrow-down-icon')
			.click()
		cy.get('.dialog__actions button')
			.contains('Move page here')
			.click()
		cy.get('.page-browser').should('not.exist')

		cy.openPage('Target')
		cy.getPageListItem('Target')
			.find('.app-navigation-entry__children .page-list-item')
			.first()
	...
	it('Moves page to a subpage in another collective', function() {
		...
		cy.getPageListItem('Target external')
			.find('.app-navigation-entry__children .page-list-item')
			.first()
	...
	it('Shows only (sub)pages matching the filter string', function() {
		cy.get('input[name="pageFilter"]')
			.type('Title')
		cy.get('.app-navigation-entry__name:visible').should('have.length', 3)
		cy.get('.app-navigation-entry__name:visible').eq(0)
			.should('contain', 'Page Title')
		cy.get('.app-navigation-entry__name:visible').eq(1)
			.should('contain', 'Subpage Title')
		cy.get('.app-navigation-entry__name:visible').eq(2)
			.should('contain', 'DeepSubpage Title')
	})
	...
	it('allows to trash and restore page with subpage and attachment', function() {
		...
		cy.get('.page-list .page-list-item')
