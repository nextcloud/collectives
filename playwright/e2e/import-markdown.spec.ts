...
test.describe('Import Markdown', () => {
	test('attached images got rewritten and render', async ({ collective, editor }) => {
		...
		await expect(collective.page).toHaveTitle(`start - ${collective.data.name} - Collectives - Nextcloud`)
		await editor.hasImage('stegosaurus.png')

		await collective.openCollective({ pageTitle: 'page1/subpage1' })
		await expect(collective.page).toHaveTitle(`subpage1 - page1 - ${collective.data.name} - Collectives - Nextcloud`)
	...
	test('table renders', async ({ collective, editor }) => {
		...
		await expect(collective.page).toHaveTitle(`page1 - ${collective.data.name} - Collectives - Nextcloud`)
