OC.L10N.register(
    "collectives",
    {
    "Add a collective" : "Add a collective",
    "Recent pages" : "Recent pages",
    "Collectives" : "Collectives",
    "{user} has mentioned you in {collective} - {page}" : "{user} has mentioned you in {collective} - {page}",
    "Landing page" : "Landing page",
    "Welcome %s" : "Welcome %s",
    "Add pages with the ➕ in the page list! You can drag them to change the order. Dragging them into the editor will create a 🔗 link." : "Add pages with the ➕ in the page list! You can drag them to change the order. Dragging them into the editor will create a 🔗 link.",
    "Press `✏️ Edit` to change this text and make yourself at home! Multiple people can edit together. Find out more in the [documentation](https://docs.nextcloud.com/server/latest/user_manual/en/collectives/index.html) or ask [the community](https://help.nextcloud.com/tag/collectives-app)." : "Press `✏️ Edit` to change this text and make yourself at home! Multiple people can edit together. Find out more in the [documentation](https://docs.nextcloud.com/server/latest/user_manual/en/collectives/index.html) or ask [the community](https://help.nextcloud.com/tag/collectives-app).",
    "{user} updated {page} in {collective}" : "{user} updated {page} in {collective}",
    "{user} deleted {page} from {collective}" : "{user} deleted {page} from {collective}",
    "Link to page in collective" : "Link to page in collective",
    "Collective pages" : "Collective pages",
    "In collective %1$s" : "In collective %1$s",
    "Last edition %1$s" : "Last edition %1$s",
    "Collectives - Page content" : "Collectives - Page content",
    "Collectives - Pages" : "Collectives - Pages",
    "Created collective \"%s\" for existing team." : "Created collective \"%s\" for existing team.",
    "Collective Share" : "Collective Share",
    "You are not allowed to share %s" : "You are not allowed to share %s",
    "You are not allowed to edit %s" : "You are not allowed to edit %s",
    "Share not found for user" : "Share not found for user",
    "Teams App Enabled" : "Teams App Enabled",
    "Teams app is enabled" : "Teams app is enabled",
    "The teams app is not enabled, but is required for Collectives to work." : "The teams app is not enabled, but is required for Collectives to work.",
    "Your space to collaboratively write and organize" : "Your space to collaboratively write and organise",
    "Your space to collaboratively write and organize. Collectives is designed for groups and communities\nto structure shared knowledge and cultivate trust.\n\n* **👥 Non-hierarchical at its core**: Each collective is backed by a\n  [Nextcloud Team](https://docs.nextcloud.com/server/latest/user_manual/en/groupware/contacts.html#teams) - its\n  content is owned by the group, not a single user.\n* **📝 Collaborative page editing** thanks to the [Text app](https://github.com/nextcloud/text).\n* **🔤 Well-known [Markdown](https://en.wikipedia.org/wiki/Markdown) syntax** for page formatting.\n* **🔎 Full-text search** to find content straight away." : "Your space to collaboratively write and organise. Collectives is designed for groups and communities\nto structure shared knowledge and cultivate trust.\n\n* **👥 Non-hierarchical at its core**: Each collective is backed by a\n  [Nextcloud Team](https://docs.nextcloud.com/server/latest/user_manual/en/groupware/contacts.html#teams) - its\n  content is owned by the group, not a single user.\n* **📝 Collaborative page editing** thanks to the [Text app](https://github.com/nextcloud/text).\n* **🔤 Well-known [Markdown](https://en.wikipedia.org/wiki/Markdown) syntax** for page formatting.\n* **🔎 Full-text search** to find content straight away.",
    "All changes" : "All changes",
    "@-mentions only" : "@-mentions only",
    "Off" : "Off",
    "Extra action" : "Extra action",
    "You left collective {name}" : "You left collective {name}",
    "Could not leave the collective. {errorMessage}" : "Unable to leave the collective: {errorMessage}",
    "Manage members" : "Manage members",
    "Share link" : "Share link",
    "Manage templates" : "Manage templates",
    "Export or print" : "Export or print",
    "Settings" : "Settings",
    "Notifications" : "Notifications",
    "Leave collective" : "Leave collective",
    "Back" : "Back",
    "Offline" : "Offline",
    "Collective not found: {collective}" : "Collective not found: {collective}",
    "Collective with ID {id} not found" : "Collective with ID {id} not found",
    "You're not part of a collective with that name." : "You're not part of a collective with that name.",
    "Loading pages:" : "Loading pages:",
    "Loading images:" : "Loading images:",
    "Preparing collective for exporting or printing" : "Preparing collective for exporting or printing",
    "Last changed by" : "Last changed by",
    "Members" : "Members",
    "No search results" : "No search results",
    "admin" : "admin",
    "moderator" : "moderator",
    "member" : "member",
    "Demote to moderator" : "Demote to moderator",
    "Promote to moderator" : "Promote to moderator",
    "Member level changed" : "Member level changed",
    "Could not change member level" : "Could not change member level",
    "Member removed" : "Member removed",
    "Could not remove member" : "Could not remove member",
    "Promote to admin" : "Promote to admin",
    "Demote to member" : "Demote to member",
    "Remove" : "Remove",
    "An error occurred while performing the search" : "An error occurred while performing the search",
    "Search accounts, groups, teams" : "Search accounts, groups, teams",
    "Add accounts, groups or teams" : "Add accounts, groups or teams",
    "Search for members to add." : "Search for members to add.",
    "Loading…" : "Loading…",
    "Add accounts" : "Add accounts",
    "Add groups" : "Add groups",
    "Add teams" : "Add teams",
    "Remove {name}" : "Remove {name}",
    "Members of collective {name}" : "Members of collective {name}",
    "Go to team overview" : "Go to team overview",
    "Select a collective" : "Select a collective",
    "Collective actions" : "Collective actions",
    "New collective" : "New collective",
    "Change emoji" : "Change emoji",
    "Add emoji" : "Add emoji",
    "Name too short, requires at least three characters" : "Name too short, requires at least three characters",
    "Editing permissions updated" : "Editing permissions updated",
    "Could not update editing permissions" : "Could not update editing permissions",
    "Sharing permissions updated" : "Sharing permissions updated",
    "Could not update sharing permissions" : "Could not update sharing permissions",
    "Default page mode updated" : "Default page mode updated",
    "Could not update default page mode" : "Could not update default page mode",
    "Emoji updated" : "Emoji updated",
    "Could not update emoji for the collective" : "Could not update emoji for the collective",
    "Could not rename the collective" : "Unable to rename the collective",
    "Collective settings" : "Collective settings",
    "Name and emoji" : "Name and emoji",
    "Select emoji for collective" : "Select emoji for collective",
    "Name of the collective" : "Name of the collective",
    "Permissions" : "Permissions",
    "Allow editing for" : "Allow editing for",
    "Admins only" : "Admins only",
    "Admins and moderators" : "Admins and moderators",
    "All members" : "All members",
    "Allow sharing for" : "Allow sharing for",
    "Page settings" : "Page settings",
    "Default Page Mode" : "Default Page Mode",
    "Preview" : "Preview",
    "Edit" : "Edit",
    "Danger zone" : "Danger zone",
    "Delete collective" : "Delete collective",
    "Deleted collectives can be restored from the collectives trash." : "Deleted collectives can be restored from the collectives trash.",
    "Create a collective first" : "Create a collective first",
    "Select location for collectives" : "Select location for collectives",
    "Choose" : "Choose",
    "Invalid path selected. Only folders on first level are supported." : "Invalid path selected. Only folders on first level are supported.",
    "Collectives settings" : "Collectives settings",
    "Collectives folder" : "Collectives folder",
    "Deleted collectives" : "Deleted collectives",
    "No deleted collectives." : "No deleted collectives.",
    "Name" : "Surname",
    "Deleted" : "Deleted",
    "Restore collective" : "Restore collective",
    "Restore" : "Restore",
    "Delete permanently" : "Delete permanently",
    "Permanently delete collective {collective}" : "Permanently delete collective {collective}",
    "Delete corresponding team along with the collective?" : "Delete corresponding team along with the collective?",
    "Delete only collective" : "Delete only collective",
    "Only collective" : "Only collective",
    "Delete collective and team" : "Delete collective and team",
    "Collective and team" : "Collective and team",
    "Only team owners can delete a team" : "Only team owners can delete a team",
    "Cancel" : "Cancel",
    "Add members to {name}" : "Add members to {name}",
    "Name too long, must not exceed 127 characters" : "Name too long, must not exceed 127 characters",
    "A collective/team with this name already exists" : "A collective/team with this name already exists",
    "Create" : "Create",
    "Create without members" : "Create without members",
    "Could not add members to the collective" : "Could not add members to the collective",
    "Select emoji" : "Select emoji",
    "Select an existing team" : "Select an existing team",
    "Select a team…" : "Select a team…",
    "Cancel selecting a team" : "Cancel selecting a team",
    "Add members" : "Add members",
    "Go back" : "Go back",
    "Delete template and subpages" : "Delete template and subpages",
    "Delete" : "Delete",
    "Could not save emoji for template" : "Could not save emoji for template",
    "Could not rename template" : "Could not rename template",
    "Select emoji for template" : "Select emoji for template",
    "Template title" : "Template title",
    "Rename" : "Rename",
    "Could not create template" : "Could not create template",
    "Could not delete template" : "Could not delete template",
    "Templates" : "Templates",
    "Add a template" : "Add a template",
    "Stop editing" : "Stop editing",
    "Start editing" : "Start editing",
    "Collapse members" : "Collapse members",
    "Expand members" : "Expand members",
    "Show members" : "Show members",
    "Manage members of the collective" : "Manage members of the collective",
    "Show all members of the collective" : "Show all members of the collective",
    "Could not save show members setting for collective" : "Could not save show members setting for collective",
    "Collapse recent pages" : "Collapse recent pages",
    "Expand recent pages" : "Expand recent pages",
    "Could not save recent pages setting for collective" : "Could not save recent pages setting for collective",
    "Scroll recent pages to the left" : "Scroll recent pages to the left",
    "Hide outline" : "Hide outline",
    "Show outline" : "Show outline",
    "Remove from favorites" : "Remove from favourites",
    "Add to favorites" : "Add to favourites",
    "Delete page and subpages" : "Delete page and subpages",
    "Delete page" : "Delete page",
    "Open page sidebar" : "Open page sidebar",
    "Full width" : "Full width",
    "Manage tags" : "Manage tags",
    "Move or copy" : "Move or copy",
    "Download" : "Download",
    "All collectives" : "All collectives",
    "Breadcrumb for page {name}" : "Breadcrumb for page {name}",
    "Breadcrumb for collective {name}" : "Breadcrumb for collective {name}",
    "Breadcrumb for list of collectives" : "Breadcrumb for list of collectives",
    "Breadcrumb, navigate to page {page}" : "Breadcrumb, navigate to page {page}",
    "Move page here" : "Move page here",
    "Move page to {collective}" : "Move page to {collective}",
    "Copy page here" : "Copy page here",
    "Copy page to {collective}" : "Copy page to {collective}",
    "Copy or move page" : "Copy or move page",
    "Move page up" : "Move page up",
    "Move page down" : "Move page down",
    "Open version history" : "Open version history",
    "Open attachments" : "Open attachments",
    "Open backlinks" : "Open backlinks",
    "_%n attachment_::_%n attachments_" : ["%n attachment","%n attachments"],
    "_%n backlink_::_%n backlinks_" : ["%n backlink","%n backlinks"],
    "Page not found: {page}" : "Page not found: {page}",
    "Page with ID {id} not found" : "Page with ID {id} not found",
    "Select a page from the list or create a new one." : "Select a page from the list or create a new one.",
    "Filter page list by tag" : "Filter page list by tag",
    "Could not rename the page" : "Could not rename the page",
    "Select emoji for page" : "Select emoji for page",
    "Title" : "Title",
    "Open sidebar" : "Open sidebar",
    "Clear search" : "Clear search",
    "Find previous match" : "Find previous match",
    "Find previous" : "Find previous",
    "Find next match" : "Find next match",
    "Find next" : "Find next",
    "Found {matches} matches for \"{query}\"" : "Found {matches} matches for \"{query}\"",
    "Match {index} of {matches} for \"{query}\"" : "Match {index} of {matches} for \"{query}\"",
    "Highlight all matches" : "Highlight all matches",
    "Added tag {name} to page" : "Added tag {name} to page",
    "Could not add tag {name} to page" : "Could not add tag {name} to page",
    "Removed tag {name} from page" : "Removed tag {name} from page",
    "Could not remove tag {name} from page" : "Could not remove tag {name} from page",
    "Updated tag {name}" : "Updated tag {name}",
    "Could not update tag {name}" : "Could not update tag {name}",
    "Could not delete tag {name}" : "Could not delete tag {name}",
    "_Deleted %n tag_::_Deleted %n tags_" : ["Deleted %n tag","Deleted %n tags"],
    "Created tag {name}" : "Created tag {name}",
    "Could not create tag {name}" : "Could not create tag {name}",
    "Search or create tag" : "Search or create tag",
    "Tag name" : "Tag name",
    "Change tag color" : "Change tag colour",
    "Create new tag {tag}" : "Create new tag {tag}",
    "Create new tag" : "Create new tag",
    "_(removed from %n page)_::_(removed from %n pages)_" : ["(removed from %n page)","(removed from %n pages)"],
    "Error saving the document. Please try again." : "Error saving the document. Please try again.",
    "Blank page" : "Blank page",
    "Could not create new page" : "Could not create new page",
    "Pick a template" : "Pick a template",
    "Favorites" : "Favourites",
    "Toggle favorites" : "Toggle favorites",
    "Favorite" : "Favourite",
    "Add a subpage" : "Add a subpage",
    "Page permanently deleted" : "Page permanently deleted",
    "Show deleted pages" : "Show deleted pages",
    "Deleted pages" : "Deleted pages",
    "No deleted pages." : "No deleted pages.",
    "Restore page" : "Restore page",
    "Sorted ascending by title" : "Sorted ascending by title",
    "Sorted descending by title" : "Sorted descending by title",
    "Sorted by recently changed" : "Sorted by recently changed",
    "Sorted by least recently changed" : "Sorted by least recently changed",
    "Could not save page order for collective" : "Could not save page order for collective",
    "Search pages" : "Search pages",
    "Search pages…" : "Search pages…",
    "Sort order" : "Sort order",
    "Sort by custom order" : "Sort by custom order",
    "Sort least recently changed first" : "Sort least recently changed first",
    "Sort recently changed first" : "Sort recently changed first",
    "Sort descending by title" : "Sort descending by title",
    "Sort ascending by title" : "Sort ascending by title",
    "Add a page" : "Add a page",
    "Switch back to default sort order" : "Switch back to default sort order",
    "Results in title or tags" : "Results in title or tags",
    "Results in content" : "Results in content",
    "Page not found" : "Page not found",
    "The page does not exist or you are not allowed to view it." : "The page does not exist or you are not allowed to view it.",
    "View in page" : "View in page",
    "Add to page" : "Add to page",
    "Show in Files" : "Show in Files",
    "Rename attachment" : "Rename attachment",
    "Attachment name" : "Attachment name",
    "Make sure you are online and can reach the server." : "Make sure you are online and can reach the server.",
    "Create public link" : "Create public link",
    "Share link ({index})" : "Share link ({index})",
    "You are offline" : "You are offline",
    "View only" : "View only",
    "Can edit" : "Can edit",
    "Cannot copy, please copy the link manually" : "Cannot copy, please copy the link manually",
    "Copy public link to clipboard" : "Copy public link to clipboard",
    "Share actions" : "Share actions",
    "Password field can't be empty" : "Password field can't be empty",
    "Page \"{name}\" has been shared" : "Page \"{name}\" has been shared",
    "Collective \"{name}\" has been shared" : "Collective \"{name}\" has been shared",
    "Failed to share page \"{name}\": {responseError}" : "Failed to share page \"{name}\": {responseError}",
    "Failed to share collective \"{name}\": {responseError}" : "Failed to share collective \"{name}\": {responseError}",
    "Share link of page \"{name}\" has been updated" : "Share link of page \"{name}\" has been updated",
    "Share link of collective \"{name}\" has been updated" : "Share link of collective \"{name}\" has been updated",
    "Failed to update share link: {responseError}" : "Failed to update share link: {responseError}",
    "Page \"{name}\" has been unshared" : "Page \"{name}\" has been unshared",
    "Collective \"{name}\" has been unshared" : "Collective \"{name}\" has been unshared",
    "Failed to unshare: {responseError}" : "Failed to unshare: {responseError}",
    "Error generating password from password policy" : "Error generating password from password policy",
    "Quick share options dropdown" : "Quick share options dropdown",
    "Please enter the following required information before creating the share" : "Please enter the following required information before creating the share",
    "Password protection (enforced)" : "Password protection (enforced)",
    "Set password" : "Set password",
    "Password" : "Password",
    "Create share" : "Create share",
    "Add another link" : "Add another link",
    "Advanced settings" : "Advanced settings",
    "Unshare" : "Unshare",
    "Create a new share link" : "Create a new share link",
    "Update share" : "Update share",
    "Add attachments using drag and drop or via \"Insert attachment\" in the formatting menu." : "Add attachments using drag and drop or via \"Insert attachment\" in the formatting menu.",
    "Failed to upload attachment {name}" : "Failed to upload attachment {name}",
    "Failed to rename attachment" : "Failed to rename attachment",
    "Deleted attachment {name}" : "Deleted attachment {name}",
    "Failed to delete attachment" : "Failed to delete attachment",
    "Failed to restore attachment" : "Failed to restore attachment",
    "Could not get attachments" : "Could not get attachments",
    "Upload" : "Upload",
    "Drag and drop files here to upload" : "Drag and drop files here to upload",
    "Uploading" : "Uploading",
    "In page" : "In page",
    "Not in page" : "Not in page",
    "Recently deleted" : "Recently deleted",
    "Found in folder" : "Found in folder",
    "Explanation for found in folder list" : "Explanation for found in folder list",
    "Files in the folder of the page." : "Files in the folder of the page.",
    "No attachments" : "No attachments",
    "No backlinks available" : "No backlinks available",
    "If other pages link to this one, they will be listed here." : "If other pages link to this one, they will be listed here.",
    "Manage members of your collective. Members have access to the whole collective." : "Manage members of your collective. Members have access to the whole collective.",
    "Share this collective with others outside your collective via public links. Anyone with the link can access the shared content." : "Share this collective with others outside your collective via public links. Anyone with the link can access the shared content.",
    "Share this page with others outside your collective via public links. Anyone with the link can access the shared content." : "Share this page with others outside your collective via public links. Anyone with the link can access the shared content.",
    "Collective members" : "Collective members",
    "Member management explanation" : "Member management explanation",
    "Manage collective members" : "Manage collective members",
    "External shares" : "External shares",
    "External shares explanation" : "External shares explanation",
    "Could not get page versions" : "Could not get page versions",
    "Compare versions…" : "Compare versions…",
    "Page versions" : "Page versions",
    "No other versions available" : "No other versions available",
    "After editing you can find old versions of the page here." : "After editing you can find old versions of the page here.",
    "This page version cannot be used for comparison." : "This page version cannot be used for comparison.",
    "Ambiguous version" : "Ambiguous version",
    "Unavailable version ({version})" : "Unavailable version ({version})",
    "The version comparison link is ambiguous and could not be opened." : "The version comparison link is ambiguous and could not be opened.",
    "The selected versions have expired or were removed." : "The selected versions have expired or were removed.",
    "One of the selected versions has expired or was removed." : "One of the selected versions has expired or was removed.",
    "{date} at {time}" : "{date} at {time}",
    "Current version — {timestamp}" : "Current version — {timestamp}",
    "{label} — {timestamp}" : "{label} — {timestamp}",
    "Version comparison requires a newer version of the Text app on mobile." : "Version comparison requires a newer version of the Text app on mobile.",
    "Version comparison is not supported by the installed Text app." : "Version comparison is not supported by the installed Text app.",
    "Could not update the comparison link." : "Could not update the comparison link.",
    "Comparison link copied" : "Comparison link copied",
    "Could not copy the comparison link." : "Could not copy the comparison link.",
    "The selected version is too large for complete comparison. A bounded preview is shown instead." : "The selected version is too large for complete comparison. A bounded preview is shown instead.",
    "Could not save current changes before comparison. Please try again." : "Could not save current changes before comparison. Please try again.",
    "Could not initialize version comparison." : "Could not initialize version comparison.",
    "You do not have permission to load one of the selected versions." : "You do not have permission to load one of the selected versions.",
    "You do not have permission to load the selected versions." : "You do not have permission to load the selected versions.",
    "One of the selected versions contains invalid UTF-8 text." : "One of the selected versions contains invalid UTF-8 text.",
    "Could not load the selected versions because of a network error." : "Could not load the selected versions because of a network error.",
    "Could not clean up version comparison." : "Could not clean up version comparison.",
    "Compare versions" : "Compare versions",
    "Earlier" : "Earlier",
    "Later" : "Later",
    "Copy comparison link" : "Copy comparison link",
    "Some page versions could not be used for comparison." : "Some page versions could not be used for comparison.",
    "Select two different versions." : "Select two different versions.",
    "Bounded version previews" : "Bounded version previews",
    "Could not load a preview for this version." : "Could not load a preview for this version.",
    "Open version" : "Open version",
    "Download version" : "Download version",
    "Loading versions…" : "Loading versions…",
    "Retry" : "Retry",
    "Compare" : "Compare",
    "Current version" : "Current version",
    "Initial version" : "Initial version",
    "You" : "You",
    "Actions for versions from {versionHumanExplicitDate}" : "Actions for versions from {versionHumanExplicitDate}",
    "Name this version" : "Name this version",
    "Edit version name" : "Edit version name",
    "Compare with current version" : "Compare with current version",
    "Restore version" : "Restore version",
    "Delete version" : "Delete version",
    "Version name" : "Version name",
    "Named versions are persisted, and excluded from automatic cleanups when your storage quota is full." : "Named versions are persisted, and excluded from automatic cleanups when your storage quota is full.",
    "Remove version name" : "Remove version name",
    "Save version name" : "Save version name",
    "This version comparison link is invalid." : "This version comparison link is invalid.",
    "Version comparison is not available for public links." : "Version comparison is not available for public links.",
    "Attachments" : "Attachments",
    "Backlinks" : "Backlinks",
    "Sharing" : "Sharing",
    "Versions" : "Versions",
    "Remove tag" : "Remove tag",
    "Restore this version" : "Restore this version",
    "Empty string or path starting with \"/\" is expected" : "Empty string or path starting with \"/\" is expected",
    "Saved default user folder" : "Saved default user folder",
    "Collectives default user folder" : "Collectives default user folder",
    "The default path where collectives are mounted in user home directory" : "The default path where collectives are mounted in user home directory",
    "Default user folder" : "Default user folder",
    "The content of this folder is best viewed in the Collectives app." : "The content of this folder is best viewed in the Collectives app.",
    "Open in Collectives" : "Open in Collectives",
    "Come, organize and build shared knowledge!" : "Come organise and build shared knowledge!",
    "Create new collective" : "Create new collective",
    "In collective {path}" : "In collective {path}",
    "Add link to page" : "Add link to page",
    "Limit to current collective" : "Limit to current collective",
    "Other members may lack access to pages from other collectives." : "Other members may lack access to pages from other collectives.",
    "Loading pages" : "Loading pages",
    "No pages to link" : "No pages to link",
    "Link to page" : "Link to page",
    "Link copied" : "Link copied",
    "Could not copy link to the clipboard:" : "Could not copy link to the clipboard:",
    "New page" : "New page",
    "Could not create the page" : "Could not create the page",
    "Could not save emoji for page" : "Could not save emoji for page",
    "Could not copy page" : "Could not copy page",
    "Could not move page" : "Could not move page",
    "Could not copy page to another collective" : "Could not copy page to another collective",
    "Could not move page to another collective" : "Could not move page to another collective",
    "Could not delete the page" : "Could not delete the page",
    "Page deleted" : "Page deleted",
    "Could not change page order" : "Could not change page order",
    "Failed to restore {basename} version of {page}." : "Failed to restore {basename} version of {page}.",
    "Restored {basename} version of {page}." : "Restored {basename} version of {page}.",
    "Failed to set version label for {basename}." : "Failed to set version label for {basename}.",
    "Set label for {basename}." : "Set label for {basename}.",
    "Failed to delete {basename} version of {page}." : "Failed to delete {basename} version of {page}.",
    "Deleted {basename} version of {page}." : "Deleted {basename} version of {page}.",
    "Nextcloud Collectives was updated." : "Nextcloud Collectives was updated.",
    "Please reload the page." : "Please reload the page.",
    "Error: Missing apps" : "Error: Missing apps",
    "The following dependency apps are missing:" : "The following dependency apps are missing:",
    "Please ask the administrator to enable these apps." : "Please ask the administrator to enable these apps.",
    "in Collective {collective}" : "in Collective {collective}",
    "Created collective \"%s\" for existing circle." : "Created collective \"%s\" for existing circle.",
    "A share for collective %s exists already" : "A share for collective %s exists already",
    "A place for activist and community projects to build shared knowledge" : "A place for activist and community projects to build shared knowledge",
    "Collectives is a Nextcloud App for activist and community projects to organize together.\nCome and gather in collectives to build shared knowledge.\n\n* 👥 **Collective and non-hierarchical workflow by heart**: Collectives are\n  tied to a [Nextcloud Circle](https://github.com/nextcloud/circles) and\n  owned by the collective.\n* 📝 **Collaborative page editing** like known from Etherpad thanks to the\n  [Text app](https://github.com/nextcloud/text).\n* 🔤 **Well-known [Markdown](https://en.wikipedia.org/wiki/Markdown) syntax**\n  for page formatting.\n\n## Installation\n\nIn your Nextcloud instance, simply navigate to **»Apps«**, find the\n**»Circles«** and **»Collectives«** apps and enable them." : "Collectives is a Nextcloud App for activist and community projects to organise together.\nCome and gather in collectives to build shared knowledge.\n\n* 👥 **Collective and non-hierarchical workflow by heart**: Collectives are\n  tied to a [Nextcloud Circle](https://github.com/nextcloud/circles) and\n  owned by the collective.\n* 📝 **Collaborative page editing** like known from Etherpad thanks to the\n  [Text app](https://github.com/nextcloud/text).\n* 🔤 **Well-known [Markdown](https://en.wikipedia.org/wiki/Markdown) syntax**\n  for page formatting.\n\n## Installation\n\nIn your Nextcloud instance, simply navigate to **»Apps«**, find the\n**»Circles«** and **»Collectives«** apps and enable them.",
    "The contacts app is required to manage members" : "The contacts app is required to manage members",
    "Copied" : "Copied",
    "Cannot copy" : "Cannot copy",
    "Copy share link" : "Copy share link",
    "Could not change the collective share editing permissions" : "Unable to change the collective share editing permissions",
    "Could not share the collective" : "Unable to share the collective",
    "Could not unshare the collective" : "Unable to unshare the collective",
    "Allow editing in share" : "Allow editing in share",
    "Collective not found:" : "Collective not found:",
    "Select a collective or create a new one on the left." : "Select a collective or create a new one on the left.",
    "Last edited by {user}" : "Last edited by {user}",
    "Renaming is limited to owners of the circle" : "Renaming is limited to owners of the circle",
    "Save new collective name" : "Save new collective name",
    "Admins and moderaters" : "Admins and moderaters",
    "Default page mode" : "Default page mode",
    "View" : "View",
    "Members can be managed via the connected circle in the Contacts app." : "Members can be managed via the connected circle in the Contacts app.",
    "Open circle in Contacts" : "Open circle in Contacts",
    "Collectives Folder" : "Collectives Folder",
    "Permanently delete collective »{collective}«" : "Permanently delete collective »{collective}«",
    "Delete corresponding circle along with the collective?" : "Delete corresponding circle along with the collective?",
    "Collective and circle" : "Collective and circle",
    "Only circle owners can delete a circle" : "Only circle owners can delete a circle",
    "Create collective for existing circle" : "Create collective for existing circle",
    "New collective name" : "New collective name",
    "Select circle..." : "Select circle...",
    "Create a new collective" : "Create a new collective",
    "Cancel creating a new collective" : "Cancel creating a new collective",
    "Edit template for subpages" : "Edit template for subpages",
    "Add template for subpages" : "Add template for subpages",
    "Cannot delete page with subpages" : "Cannot delete page with subpages",
    "Delete template" : "Delete template",
    "Page not found:" : "Page not found:",
    "Could not find the Page." : "Could not find the Page.",
    "Reverted {page} to revision {timestamp}." : "Reverted {page} to revision {timestamp}.",
    "Failed to revert {page} to revision {timestamp}." : "Failed to revert {page} to revision {timestamp}.",
    "Template" : "Template",
    "Done" : "Done",
    "Page content" : "Page content",
    "Expand subpage list" : "Expand subpage list",
    "Search pages ..." : "Search pages ...",
    "Hide templates" : "Hide templates",
    "Show templates" : "Show templates",
    "Sort by title" : "Sort by title",
    "Sorted by title" : "Sorted by title",
    "Could not get page backlinks" : "Could not get page backlinks",
    "_%n byte_::_%n bytes_" : ["%n byte","%n bytes"],
    "Pages that link to this one" : "Pages that link to this one",
    "Old versions of this page" : "Old versions of this page",
    "Link copied to the clipboard." : "Link copied to the clipboard.",
    "New Page" : "New Page",
    "The following dependency apps are missing: " : "The following dependency apps are missing: "
},
"nplurals=2; plural=(n != 1);");
