...
class ImportService {
	...
	public function importDirectory(string $directory, Collective $collective, int $parentId, IUser $user, ?callable $progressCallback = null): int {
		...
		$message = sprintf('Memory usage after importing pages: %.2fMB', (memory_get_usage() - $memory) / 1024 / 1024);
		...
		$message = sprintf('Memory usage after rewriting links and attachments: %.2fMB', (memory_get_usage() - $memory) / 1024 / 1024);
		$progressCallback('success_verbose', $message);
