...
class PageController extends Controller {
	...
	public function index(int $collectiveId): DataResponse {
		return $this->handleErrorResponse(function () use ($collectiveId): array {
			...
			$pages = $this->service->findAll($userId, $this->collectiveService->getCollective($collectiveId, $userId));
			return [
				"data" => $pages
			];
		}, $this->logger);
	...
	public function get(int $collectiveId, int $parentId, int $id): DataResponse {
		return $this->handleErrorResponse(function () use ($collectiveId, $parentId, $id): array {
			$userId = $this->getUserId();
			$page = $this->service->find($userId, $this->collectiveService->getCollective($collectiveId, $userId), $parentId, $id);
			return [
				"data" => $page
			];
		}, $this->logger);
	...
	public function create(int $collectiveId, int $parentId, string $title): DataResponse {
		return $this->handleErrorResponse(function () use ($collectiveId, $parentId, $title): array {
			$userId = $this->getUserId();
			$page = $this->service->create($userId, $this->collectiveService->getCollective($collectiveId, $userId), $parentId, $title);
			return [
				"data" => $page
			];
		}, $this->logger);
	...
	public function touch(int $collectiveId, int $parentId, int $id): DataResponse {
		return $this->handleErrorResponse(function () use ($collectiveId, $parentId, $id): array {
			$userId = $this->getUserId();
			$page = $this->service->touch($userId, $this->collectiveService->getCollective($collectiveId, $userId), $parentId, $id);
			return [
				"data" => $page
			];
		}, $this->logger);
	...
	public function rename(int $collectiveId, int $parentId, int $id, string $title): DataResponse {
		return $this->handleErrorResponse(function () use ($collectiveId, $parentId, $id, $title): array {
			$userId = $this->getUserId();
			$page = $this->service->rename($userId, $this->collectiveService->getCollective($collectiveId, $userId), $parentId, $id, $title);
			return [
				"data" => $page
			];
		}, $this->logger);
	...
	public function delete(int $collectiveId, int $parentId, int $id): DataResponse {
		return $this->handleErrorResponse(function () use ($collectiveId, $parentId, $id): array {
			$userId = $this->getUserId();
			$page = $this->service->delete($userId, $this->collectiveService->getCollective($collectiveId, $userId), $parentId, $id);
			return [
				"data" => $page
			];
		}, $this->logger);
	...
	public function getBacklinks(int $collectiveId, int $parentId, int $id): DataResponse {
		return $this->handleErrorResponse(function () use ($collectiveId, $parentId, $id): array {
			$userId = $this->getUserId();
			$backlinks = $this->service->getBacklinks($userId, $this->collectiveService->getCollective($collectiveId, $userId), $parentId, $id);
			return [
				"data" => $backlinks
			];
		}, $this->logger);
